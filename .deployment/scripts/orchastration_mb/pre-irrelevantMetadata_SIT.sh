...
# Temp remove folders that should not be considered during delta nor full deployment against SIT
rm -r fs_manual_deployment
rm -r fs_post_org-dependent/PROD
rm -r fs_post_org-dependent/STAGE
...
if [ "$POSTORG_Validation" = 'validation' ]; then
    ...
    # Temp remove folders that should not be considered during delta nor full validation against SIT
    rm -r fs_post_org-dependent/SIT/labels
