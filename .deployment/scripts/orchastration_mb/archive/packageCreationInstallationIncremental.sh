...
if [ $(echo "$changedPaths" | grep -c '^core_dm$') == 1 ]; then
    ...
    status=$(echo $json | jq '.status')
        if [ $status == "0" ]; then
            ...
            .deployment/scripts/packageInstall.sh $TARGET_ORG_ALIAS $packageVersionId_core_dm $INSTALLATIONKEY
...
#            echo "--- START Package Installation ---"
#            .deployment/scripts/packageInstall.sh $TARGET_ORG_ALIAS $packageVersionId_core_bl $INSTALLATIONKEY
...
if [ $(echo "$changedPaths" | grep -c '^fs_dm$') == 1 ]; then
    ...
    status=$(echo $json | jq '.status')
        if [ $status == "0" ]; then
            ...
            echo "--- START Package Installation ---"
            .deployment/scripts/packageInstall.sh $TARGET_ORG_ALIAS $packageVersionId_fs_dm $INSTALLATIONKEY
            echo "--- END Package Installation ---"
        else
            echo "--- sfdx package version create failed ---"
        fi
...
if [ $(echo "$changedPaths" | grep -c '^fs_bl$') == 1 ]; then
    ...
    status=$(echo $json | jq '.status')
        if [ $status == "0" ]; then
            ...
            echo "--- START Package Installation ---"
            .deployment/scripts/packageInstall.sh $TARGET_ORG_ALIAS $packageVersionId_fs_bl $INSTALLATIONKEY
            echo "--- END Package Installation ---"
        else
            echo "--- sfdx package version create failed ---"
        fi
